...
StringType::size_type ExtensionSeparatorPosition(const StringType& path) {
  ...
  for (auto* i : kCommonDoubleExtensions) {
    ...
    if (EqualsCaseInsensitiveASCII(extension, i)) return penultimate_dot;
  }
...
bool FilePath::MatchesFinalExtension(StringPieceType extension) const {
  ...
  if (current_final_extension.length() != extension.length()) return false;
...
std::string FilePath::MaybeAsASCII() const {
  if (base::IsStringASCII(path_)) return path_;
  return std::string();
}
