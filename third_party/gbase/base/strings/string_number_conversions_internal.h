...
uint8_t CharToDigit(CHAR c, bool& valid) {
   ...
   public:
    ...
    static Result Invoke(Iter begin, Iter end) {
      ...
      for (Iter current = begin; current != end; ++current) {
        bool valid = false;
        uint8_t new_digit = CharToDigit<kBase>(*current, valid);
...
static bool HexStringToByteContainer(StringPiece input, OutIter output) {
  ...
  for (uintptr_t i = 0; i < count / 2; ++i) {
    ...
    bool msb_valid = false;
    uint8_t msb = CharToDigit<16>(input[i * 2], msb_valid);
    // least significant 4 bits
    bool lsb_valid = false;
    uint8_t lsb = CharToDigit<16>(input[i * 2 + 1], lsb_valid);
