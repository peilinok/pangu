...
int record_desktop_mag::init(const RECORD_DESKTOP_RECT &rect, const int fps) {
  ...
  auto initialize_mag = [&]() {
    ...
    host_window_ =
        CreateWindowExW(WS_EX_LAYERED, kMagnifierHostClass, kHostWindowName, 0,
    ...
    magnifier_window_ = CreateWindowW(
        kMagnifierWindowClass, kMagnifierWindowName, WS_CHILD | WS_VISIBLE, 0,
        0, 0, 0, host_window_, nullptr, hInstance, nullptr);
...
int record_desktop_mag::do_record() {
  if (!magnifier_initialized_) {
  ...
  auto capture_image = [&](const RECORD_DESKTOP_RECT &rect) {
    ...
    // stuck here coz we create window with main thread
...
void record_desktop_mag::on_mag_data(void *data, const MAGIMAGEHEADER &header) {
  ...
  if (header.format != GUID_WICPixelFormat32bppRGBA ||
      ...
      header.stride != static_cast<UINT>(kBytesPerPixel) ||
